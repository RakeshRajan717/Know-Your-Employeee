...
for (var i = 0; i<icon.length; i++) {
	...
	document.getElementById('b'+i).innerHTML='<div id="" style=\'background-image: url(\"'+songs[i][2]+'\");\'>  <input type="" id="'+"a"+i+'" class="play" > <input type="" id="'+"c"+i+'" class="add">  </div><div id=""><br><br>'+songs[i][1]+'</div>';
...
async function getTime() {
                ...
                if(value=="angry"){
					...
					a = a + 1;
					var d = new Date();
				document.getElementById("a").innerHTML = "{ Seems to be anger :\ \n"+"\n Date and Time -"+d+"\n Emotion Count "+a+" }\n";
				}
                else if(value=="happy"){
                	moody(1);
					h = h+1;
					var d1 = new Date();
					document.getElementById("h").innerHTML = "{ Seems to be Happy :) \n"+"\n Date and Time -"+d1+"\n Emotion Count "+h+" }\n";
				}
                else if(value=="sad"){
                	moody(2);
					s = s+1;
					var d2 = new Date();
					document.getElementById("s").innerHTML = "{ Seems to be sad :( \n"+"\n Date and Time -"+d2+"\n Emotion Count "+s+" }\n";
				}
                else{
                	moody(3);
					n = n+1;
					var d3 = new Date();
					document.getElementById("n").innerHTML = "{ Seems to be neutral :| \n"+"\n Date and Time -"+d3+"\n Emotion Count "+n+" }\n";
				}
				
				

				if(a>h && a>s && a>n){
				document.getElementById("o").innerHTML = "The employ is anger overall";
			}
			else if(h>a && h>s && h>n){
				document.getElementById("o").innerHTML = "The employ is happy overall";
			}
			else if(s>a && s>n && s>h){
				document.getElementById("o").innerHTML = "The employ is sad overall";
			}
			else if(n>a && n>h && n>s){
				document.getElementById("o").innerHTML = "The employ is neutral overall";
			}
